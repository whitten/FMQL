...
ONEOFTYPE(REPLY,FLINF,FAR,FID,CNODESTOP) ;
 ...
 . . N FLFDK S FLFDK="" F  S FLFDK=$O(FLINF("FDPREDS",FLFDK)) Q:FLFDK=""  S BFLINF("FDPREDS",FLFDK)=FLINF("FDPREDS",FLFDK)
 ...
 . . ; Copy file-key unique predicate mapping assembled in this pass
 . . N FLFDK S FLFDK="" F  S FLFDK=$O(BFLINF("FDPREDS",FLFDK)) Q:FLFDK=""  S FLINF("FDPREDS",FLFDK)=BFLINF("FDPREDS",FLFDK)
 . E  D
...
ONEFIELD(FAR,ID,FDINF) ;
 ...
 . E  D ASSERT^FMQLJSON(REPLY,FDINF("LABEL"),FDINF("FIELD"),FDINF("TYPE"),EVALUE,,,IVALUE)
 Q
 ;
 ;
